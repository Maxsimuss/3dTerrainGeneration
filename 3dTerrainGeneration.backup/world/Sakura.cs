...
{
    class Sakura : Structure
    {
        public Sakura(int xPos, int yPos, int zPos) : base(xPos, yPos, zPos)
        {
            Random rnd = new Random(xPos + yPos + zPos);

            byte leaves = rnd.NextDouble() > .5 ? MaterialType.SAKURA_LEAVES1 : MaterialType.SAKURA_LEAVES2;
