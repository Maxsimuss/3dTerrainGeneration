...
namespace _3dTerrainGeneration.Engine.Audio
{
    ...
    internal class SoundManager
    {
        ...
        private Dictionary<SoundType, List<AudioBuffer>> buffers = new Dictionary<SoundType, List<AudioBuffer>>();
        ...
        public SoundManager(GameSettings gameSettings)
        {
            ...
            {
                buffers.Add(item, new List<AudioBuffer>());
            }
        ...
        private AudioBuffer GetBuffer(SoundType type)
        {
            List<AudioBuffer> _buffers = buffers[type];

            if (_buffers.Count < 1) return new AudioBuffer();

            return _buffers[rnd.Next(_buffers.Count)];
        }

        private AudioBuffer GenBufferFromFile(string file)
        {
